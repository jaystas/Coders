...
export async function initTTSPlayback() {
  ...
  websocket.onMessage(handleAudioStreamEvents)
...
/**
 * Handle audio stream metadata from server
 * @param {object} message
 */
function handleAudioStreamEvents(message) {
  if (message.type === 'audio_stream_start') {
    state.activeSpeaker = message.data || null
    notifySpeakerChange(state.activeSpeaker)
  }

  if (message.type === 'audio_stream_stop') {
    state.activeSpeaker = null
    notifySpeakerChange(null)
  }
}
...
/**
 * Register speaker change listener
 * @param {Function} handler - Callback(speaker: object|null)
 * @returns {Function} - Unsubscribe function
 */
export function onSpeakerChange(handler) {
  state.speakerListeners.add(handler)
  return () => state.speakerListeners.delete(handler)
}
...
/**
 * Notify all speaker listeners
 * @param {object|null} speaker
 */
function notifySpeakerChange(speaker) {
  state.speakerListeners.forEach(handler => {
    try {
      handler(speaker)
    } catch (error) {
      console.error('[TTS] Speaker listener error:', error)
    }
  })
}
